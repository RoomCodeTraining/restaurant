<x-guest-layout>
    ...
    <div class="w-full h-screen  md:flex lg:flex xl:flex">
        <div class="bg-white flex justify-center flex-col lg:w-5/12 xl:w-5/12 w-full shadow-lg">
            <div class="flex font-bold justify-center mb-6 mt-2">
                <img class="h-auto w-1/3" src="{{asset('images/logociprelvf.png')}}">
            </div>
            <div class="w-full text-center px-16">
                <form class="w-full bg-white rounded-lg"  method="POST" action="{{ route('login.store') }}">
                    @csrf
                    ...
                    <div class="px-12 pb-4">
                        <div class="w-full mt-8 mb-4">
                            <div class="flex items-center">
                                <i class="ml-3 fill-current text-gray-400 text-xs z-10 fas fa-envelope-open-text"></i>
                                <input type="email" id="tk-pages-sign-in-email" placeholder="Entrez votre email" name="email" :value="old('email')" class="-mx-6 px-8  w-full bg-opacity-25 rounded py-2 text-gray-700 focus:outline-none bg-accent-100" required autofocus  />
                            </div>
                        </div>
                        <div class="w-full mt-4">
                            <div class="flex items-center">
                                <i class='ml-3 fill-current text-gray-400 text-xs z-10 fas fa-lock'></i>
                                <input  type="password" id="tk-pages-sign-in-password" name="password" placeholder="Entrez votre mot de passe" class="-mx-6 px-8 w-full rounded py-2 text-gray-700 bg-opacity-25 focus:outline-none bg-accent-100" required autocomplete="current-password" />
                            </div>
                        </div>
                        <div class="flex items-center mt-2 mb-12">
                            <div class="w-1/2">
                                <label for="remember_me" class="inline-flex items-center">
                                    ...
                                    <span class="ml-2 text-xs text-gray-600">{{ __('Se souvenir de moi') }}</span>
                                </label>
                            </div>
                            <div class="w-1/2">
                                @if (Route::has('password.request'))
        ...
        <img src="{{asset('images/loginimg.jpg')}}" alt="background" class="object-cover object-center h-screen w-full lg:w-7/12 xl:w-7/12">
    </div>
</x-guest-layout>
