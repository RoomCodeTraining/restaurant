...
class CategoryEmployeeConsumptionChart extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        $orders = DB::table('orders')->join('users', 'orders.user_id', 'users.id')
            ->select('dish_id', 'users.userType', DB::raw('MONTH(orders.created_at) as month'), DB::raw('COUNT(*) as total_orders'))
            ->groupBy('month', 'dish_id', 'users.userType')->get();
        // ->distinct()
        // ->selectRaw('dish_id,users.user_type_id as categorie, COUNT(*) as total_orders')->get();
        //->havingRaw('COUNT(*)  < ?', [10])
