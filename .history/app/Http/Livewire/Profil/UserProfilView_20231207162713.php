        ...
        return $infolist->record($this->user)->schema([
            Section::make('Les informations de votre compte')
                ...
                ->schema([
                    ...
                    TextEntry::make('contact')->label('Contact')->formatStateUsing(function ($record) {
                    ...
                    TextEntry::make('accessCard.quota_breakfast')->label('Quota petit déjeuner')
                        ->formatStateUsing(function (User $record) {

                            return $record->accessCard->quota_breakfast ? $record->accessCard->quota_breakfast : 'aucun';
                        }),
                    TextEntry::make('accessCard.quota_lunch')->label('Quota déjeuner')
                        ->formatStateUsing(function ($record) {
                            return $record->accessCard->quota_lunch ? $record->accessCard->quota_lunch : '0';
                        }),

                    TextEntry::make('accessCard.identifier')->label('Numéro de la carte')
                        ->formatStateUsing(function ($record) {
                            return $record->accessCard->identifier ? $record->accessCard->identifier : 'Non défini';
                        }),
                    TextEntry::make('accessCard.paymentMethod.name')->label('Mode de paiement')
                        ->formatStateUsing(function ($record) {
                    ...
                    //         Tabs\Tab::make('Information sur ma carte de commande')
                    //             ->icon('heroicon-m-user')
