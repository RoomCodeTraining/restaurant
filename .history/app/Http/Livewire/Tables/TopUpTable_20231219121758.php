    ...
    public function table(Table $table): Table
    {
        return $table->query(
            ...
            Filter::make('payment')
                ...
                ])->formatStateUsing(fn (PaymentMethod $record) => $record)
            // ->query(function ($query, array $data) {
            //     //dd($data['accessCard']['user']);
            //     if ($data['accessCard']['user'] == 'C') {
            //         return $query
            //             ->where('payment', 'not like', '%' . 'P' . '%')
            //             ->where('payment', 'not like', '%' . 'S' . '%');
            //     }
            //     return $query->where('accessCard.user', 'like', '%' . $data['accessCard']['user'] . '%');
            // })
            // ->indicateUsing(function (array $data): array {
            //     $indicators = [];
