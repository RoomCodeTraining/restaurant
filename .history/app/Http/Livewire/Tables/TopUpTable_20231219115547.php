            ...
            Filter::make('payment')
                ...
                ->query(function ($query, array $data) {
                    if ($data['accessCard.paymentMethod.name'] == 'C') {
                        return $query
                            ->where('accessCard.paymentMethod.name', 'not like', '%' . 'P' . '%')
                            ->where('accessCard.paymentMethod.name', 'not like', '%' . 'S' . '%');
                    }
                    return $query->where('accessCard.paymentMethod.name', 'like', '%' . $data['accessCard.paymentMethod.name'] . '%');
                })
                ->indicateUsing(function (array $data): array {
                    $indicators = [];
                    if ($data['accessCard.paymentMethod.name'] == 'C') {
                        $indicators['accessCard.paymentMethod.name'] = ' Le mode de paiment est :  ' . 'Cash';
                    }
                    if ($data['accessCard.paymentMethod.name'] == 'P') {
                        $indicators['accessCard.paymentMethod.name'] = 'Le mode de paiement est ' . 'Postpaid';
                    }
                    if ($data['accessCard.paymentMethod.name'] == 'S') {
                        $indicators['accessCard.paymentMethod.name'] = 'Le mode de paiement est ' . 'Subvention';
                    }
