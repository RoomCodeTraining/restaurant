(function () {
	// Configs:
	const selectorForCheckboxes = 'input[type="checkbox"].indeterminate-checkbox';
	const stepsFromCheckboxToContainer = 1;
	const stepsFromContainerToParentContainer = 2;

	window.IndeterminateCheckbox = {
		init: function () {
			// Hook the onchange event for all checkboxes of the appropriate class
			$(selectorForCheckboxes).change(function (e) {
				const checkStateOfTarget = $(this).prop('checked'),
				...
				function checkRelatives(el) {
					...
					const parent = getParentOfCurrentContainer(el);
					const doTargetSiblingsOrParentSiblingsMatchTargetCheckState =
						checkIfSiblingsToEitherTargetOrParentsAreTheSameCheckStateAsTarget(el);
					...
					function checkIfSiblingsToEitherTargetOrParentsAreTheSameCheckStateAsTarget(elContainer) {
						...
						elContainer.siblings().each(function () {
							const doesSiblingStateMatchesTarget =
								getCheckboxOfContainer($(this)).prop('checked') === checkStateOfTarget;

							if (!doesSiblingStateMatchesTarget) onesNotRight = true;
						});
				...
				function traverseDOMUpwards(startingEl, steps) {
					...
					for (let i = 0; i < steps; i++) el = el.parent();
				...
				function getCheckboxOfContainer(elContainer) {
					...
					const s = stepsFromCheckboxToContainer - 1;
				...
				function getAllDirectAnccestorCheckboxes(elContainer) {
					let childCheckbox = elContainer.parents('li');
