...
class HighConsumptionChart extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        $mouvments = Order::query()->where('dish_id', '!=', null)->whereNotState('state', [Cancelled::class, Suspended::class])->get()->groupBy(function ($mouvment) {
            return $mouvment->created_at->format('M');
        });
        ...
        return [
            'datasets' => [
                [
                    'label' => $labels,
                    // 'data' => $data,
                    'data' => $mouvments->map(function ($mouvment) {
